...
function paint(){
  ...
  avatarNodes().forEach(a=>{
    ...
    a.tabIndex=0;
    a.setAttribute("aria-label","Ouvrir mon compte");
    a.title="Mon compte";
  });
}
function openAccount(){location.href="mon-compte.html"}
document.addEventListener("click",e=>{const a=e.target.closest?.(".hc-id-card .hc-avatar");if(!a)return;e.preventDefault();openAccount()},true);
document.addEventListener("keydown",e=>{if((e.key==="Enter"||e.key===" ")&&e.target?.matches?.(".hc-id-card .hc-avatar")){e.preventDefault();openAccount()}});
