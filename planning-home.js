(() => {
  ...
  function monthPanel(k) {
    ...
    for (let i = 1; i <= days; i++) {
      const dk = `${k}-${String(i).padStart(2, "0")}`,
        ...
        weekend = dow === 0 || dow === 6;
      cells += `<div class="ph-day-cell stip-month-day ${dk === today ? "today is-today" : ""} ${weekend ? "weekend is-weekend" : ""}" data-ph-date="${dk}" data-ph-month="${k}"><b class="stip-month-day-number">${i}</b><div class="ph-shift-slot stip-month-primary">${shiftVisual(code)}</div><small class="stip-month-events" aria-hidden="true"></small></div>`;
    }
    const used = pad + days,
      tail = (7 - (used % 7)) % 7;
  ...
  function renderCalendar() {
    const c = $("#phContent");
    if (!c) return;
    c.innerHTML = `<div class="ph-dedicated-intro"><span>CALENDRIER DU TÉLÉPHONE</span><h2>Synchroniser mon calendrier</h2><p>Choisis directement ce que tu veux suivre. Les abonnements restent à jour lorsque le planning change.</p></div><div class="ph-calendar-cards"><div id="phCalendarSubscriptions" class="ph-calendar-subscriptions"><div class="ph-page-load">Chargement des calendriers…</div></div><div class="ph-info-card"><strong>Mise à jour automatique</strong><small>Une fois l’abonnement ajouté, STIP maintient le calendrier à jour.</small></div></div>`;
    const host = c.querySelector("#phCalendarSubscriptions");
    if (!window.STIPCalendars?.mount?.(host)) {
      window.addEventListener(
        "stip:calendar-ready",
        () => window.STIPCalendars?.mount?.(host),
        { once: true },
      );
    }
    window.dispatchEvent(
