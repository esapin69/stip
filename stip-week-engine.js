(() => {
  ...
  function display(model={},options={}){const s=normalize(model,options),dates=visibleDates(s,{today:s.today}),dow=dateObj(s.today).getDay()||7,liveTail=s.weekOffset===0&&!s.weekFull&&!s.weekPast&&dates[0]===s.today&&dow>=5,nextMonday=liveTail&&dates.length?addDays(dates[dates.length-1],1):"";return{dates,nextMonday,visualDates:nextMonday?[...dates,nextMonday]:dates.slice(),slotCount:Math.max(1,dates.length+(nextMonday?2:0)),weekOffset:s.weekOffset,weekPast:s.weekPast,weekFull:s.weekFull,dayFocus:s.dayFocus}}
