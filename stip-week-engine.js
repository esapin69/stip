(() => {
  ...
  // CONTRAT CALENDRIER STIP — source de vérité unique.
  // Vendredi / samedi / dimanche de la semaine courante: afficher les jours restants,
  // puis une case-pont "LUNDI →", puis le lundi suivant. Les consommateurs ne
  // doivent jamais recalculer localement cette règle.
  function display(model={},options={}){
    const s=normalize(model,options),
      dates=visibleDates(s,{today:s.today}),
      dow=dateObj(s.today).getDay()||7,
      liveTail=s.weekOffset===0&&!s.weekFull&&!s.weekPast&&dates[0]===s.today&&dow>=5,
      nextMonday=liveTail&&dates.length?addDays(dates[dates.length-1],1):"";
    return{
      dates,
      nextMonday,
      visualDates:nextMonday?[...dates,nextMonday]:dates.slice(),
      // Le pont visuel consomme une colonne propre en plus de la carte du lundi.
