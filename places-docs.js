...
function pushWaypoint(routeId,wp){
  ...
  stack.push({
    routeId,
    destinationPlaceId:cfg.destination?.placeId||'',
    destinationLabel:cfg.destination?.label||'destination',
    waypointPlaceId:wp.placeId,
    waypointLabel:wp.label,
    resumeHash:location.hash||`#/place/${encodeURIComponent(cfg.destination?.placeId||'')}`
  });
  writeStack(stack);
...
function renderWaypointContext(){
  ...
  hero.insertAdjacentHTML('afterend',`<section class="waypoint-context" data-waypoint-context><span>ÉTAPE PRÉALABLE</span><strong>Rejoins ${ctx.waypointLabel}</strong><p>Tu es toujours en route vers <b>${ctx.destinationLabel}</b>. Consulte ici les informations et liaisons de ce repère. Une fois arrivé, reprends automatiquement ton trajet initial.</p><div><button type="button" data-waypoint-resume>Je suis arrivé · reprendre ${ctx.destinationLabel}</button><button type="button" data-waypoint-cancel>Annuler ce détour</button></div></section>`);
}
function enhance(){
  adaptSources();
  document.querySelectorAll('.route-card button[data-route]').forEach(btn=>adaptRoute(btn.closest('.route-card'),btn.dataset.route));
  renderWaypointContext();
}
