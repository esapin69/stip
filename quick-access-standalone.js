...
function favorite(){const apps=read().apps||{};return Object.keys(apps).filter(k=>META[k]).sort((a,b)=>Number(apps[b]||0)-Number(apps[a]||0))[0]||''}
function last(){const k=read().last||'';return META[k]?k:''}
if(META[CURRENT])touch(CURRENT);
const n=document.createElement('nav');n.id='stipQuickSwitch';n.className='stip-quick-switch';n.setAttribute('aria-label','Accès rapides STIP');const fav=favorite(),lst=CURRENT||last();n.innerHTML=`<a href="index.html?quick=public" aria-label="Accueil STIP"><span class="qs-icon">⌂</span></a><a href="index.html?quick=profile" aria-label="Ma fiche"><span class="qs-icon">◉</span></a><a ${fav?`href="${META[fav].href}"`:'aria-disabled="true"'} aria-label="Application favorite"><span class="qs-icon">★</span></a><a ${lst&&META[lst]?`href="${META[lst].href}"`:'aria-disabled="true"'} aria-label="Dernière application"><span class="qs-icon">↶</span></a>`;document.body.appendChild(n);document.body.classList.add('stip-quick-connected');
})();