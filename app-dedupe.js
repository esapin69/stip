...
function target(el){if(el.matches('a[href]'))return `url:${normUrl(el.getAttribute('href'))}`;if(el.dataset.app)return `app:${el.dataset.app}`;if(el.dataset.openPanel)return `panel:${el.dataset.openPanel}`;if(el.dataset.respSelect)return `resp:${el.dataset.respSelect}`;if(el.hasAttribute('data-resp-agents'))return 'resp:agents';if(el.hasAttribute('data-resp-agenda'))return 'resp:agenda';return''}
