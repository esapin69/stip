(()=>{'use strict';
const loaded=new Map(),V='20260824-lazy1';
...
const personal=['planning-home.js','planning-month-hero.js','planning-print-reference.js','agent-week-view.js','planning-agenda-extras.js','calendar-subscriptions.js','calendar-responsable-gate.js','day-workflow.js','day-workflow-leave.js','day-workflow-home-bridge.js','signature-success-ui.js','ui-polish-final.js'];
const team=['planning-home.js','planning-hub-enhance.js','calendar-subscriptions.js','calendar-responsable-gate.js','ui-polish-final.js'];
const change=['planning-home.js','change-workflow.js','ui-polish-final.js'];
let busyRoute='';
...
/* Stubs immédiats: les tuiles peuvent être touchées avant le téléchargement des modules. */
...
/* Les enrichissements de l'accueil arrivent après son premier affichage, sans bloquer la connexion. */
...
/* Anticipation au premier contact du doigt avec les tuiles lourdes. */
document.addEventListener('pointerdown',e=>{const b=e.target.closest?.('[data-app]');if(!b)return;const k=b.dataset.app;if(k==='responsable'){const l=document.createElement('link');l.rel='prefetch';l.href='/responsable.html';document.head.appendChild(l);return}if(k==='contacts')seq(['section-hubs.js']).catch(()=>{});if(k==='personal')seq(['planning-home.js','planning-month-hero.js']).catch(()=>{});if(k==='team')seq(['planning-home.js','planning-hub-enhance.js']).catch(()=>{});if(k==='change')seq(['planning-home.js','change-workflow.js']).catch(()=>{})},{passive:true,capture:true});
/* Si l'URL arrive directement sur une route, on la prépare. */
