...
const loginView=$('#loginView'),appView=$('#appView'),loginForm=$('#loginForm'),accessCode=$('#accessCode'),loginMessage=$('#loginMessage'),logoutBtn=$('#logoutBtn'),welcomeText=$('#welcomeText'),moduleGrid=$('#moduleGrid'),emptyPermissions=$('#emptyPermissions');
let session=null,lastRestored='';
const modules=[
 {key:'planning',num:'01',title:'Planning',small:'Planning · équipe · changements · calendrier',route:'planning'},
 {key:'equipe_contacts',num:'02',title:'Équipe & contacts',small:'Annuaire et numéros utiles',route:'contacts'},
 {key:'responsable',num:'03',title:'Responsable',small:'Suivi · évaluations · formations · stagiaires · notes',href:'responsable.html'},
 {key:'nouveaux_arrivants',num:'04',title:'Nouvel agent',small:'Parcours nouvel arrivant',href:'https://esapin69.github.io/Ghe-interne/'},
 {key:'admin',num:'05',title:'Admin',small:'Accès · imports · administration',href:'https://admin-ghe.esapin.com/'}
];
...
function planningHeader(kind){const names={personal:['MON PLANNING','Mes prochains jours'],spirit:['MON ÉQUIPE','Qui travaille aujourd’hui ?'],change:['DEMANDE','Demande de changement'],finder:['RECHERCHE','Trouver un horaire']};const n=names[kind]||['PLANNING','Planning'];$('#planningDetailKicker').textContent=n[0];$('#planningDetailTitle').textContent=n[1];showOnly('planningDetailView')}
...
$('#planningBack')?.addEventListener('click',e=>{e.preventDefault();$('#planningWorkspace').innerHTML='';setRoute('planning')});
...
(async()=>{if(!token()){showLogin();return}try{renderSession(await access('me'))}catch{localStorage.removeItem(STORAGE);showLogin('Reconnecte-toi.')}})();
})();