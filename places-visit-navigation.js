...
const DIRECTIONAL=new Set(['route_next','route_branch','exit_near','opposite_side_landmark','connects_to']);
...
function relationData(){return Array.isArray(window.__STIP_PLACE_RELATIONS)?window.__STIP_PLACE_RELATIONS:[]}
function placeData(){return window.__STIP_PLACE_DATA||null}
function hasTag(id,tag){return !!placeData()?.tags?.some(t=>t.place_id===id&&t.tag===tag)}
...
function isProfessionalElevatorRow(row){return classify(row)==='elevator'&&hasTag(row.dataset.place||'','elevator:professional')}
...
function enhanceHospital(){
  const content=q('#placesContent');if(!content||content.querySelector('.visit-hospital-start'))return;const hero=content.querySelector('.place-hero-card');if(!hero)return;const kicker=norm(hero.querySelector('.place-kicker')?.textContent);if(!kicker.includes('hôpital')&&!kicker.includes('hopital'))return;
  ...
  anchor.before(start);
  if(kind==='hfme'&&levels.length){const sec=title('Étages déjà documentés','Parcours depuis l’entrée encore partiel','visit-hfme-fallback');levels.forEach(r=>sec.querySelector('.places-list').appendChild(r));anchor.before(sec)}
  old.remove();
  if(!hero.querySelector('.visit-hospital-help')){const n=document.createElement('p');n.className='place-details-note visit-hospital-help';n.textContent=kind==='pw'?'Commence par l’entrée A ou B. Les deux zones sont reliées intérieurement.':kind==='hfme'?'Commence par l’entrée principale. Les ascenseurs colorés visiteurs restent uniquement des repères ; l’accès professionnel est indiqué séparément quand il est documenté.':'Commence par l’entrée principale puis avance de repère en repère.';hero.appendChild(n)}
}
function enhanceContextHeadings(){const content=q('#placesContent');if(!content)return;const {kicker,name}=currentContext();for(const sec of content.querySelectorAll('.places-section')){if(sec.classList.contains('visit-nav-section'))continue;const h=sec.querySelector('h2');if(!h)continue;const t=norm(h.textContent);if(t==='continuer la visite'||t==='dans ce lieu')h.textContent=childTitle(kicker,name);else if(t.includes('à proximité')||t.includes('relié à')||t.includes('ce que tu croises'))h.textContent=name.includes('hall')?'Autour du hall':'À proximité'}}
function forwardDirectionalRelation(row){const from=currentPlaceId(),to=row.dataset.place||'';if(!from||!to)return null;return relationData().filter(r=>r.from_place_id===from&&r.to_place_id===to&&DIRECTIONAL.has(r.relation_type)&&r.direction).sort((a,b)=>(Number(a.sort_order)||0)-(Number(b.sort_order)||0))[0]||null}
function directionForRow(row){const r=forwardDirectionalRelation(row);if(!r)return'';const t=norm(r.direction);const hits=[];if(/à gauche|a gauche|tourner à gauche|tourner a gauche|sur la gauche/.test(t))hits.push('left');if(/à droite|a droite|tourner à droite|tourner a droite|sur la droite/.test(t))hits.push('right');if(/tout droit|en face|devant toi|devant/.test(t))hits.push('straight');return [...new Set(hits)].length===1?hits[0]:''}
function directionLabel(k){return k==='left'?'← À gauche':k==='straight'?'↑ Tout droit':'À droite →'}
function sanitizeReverseDirections(){
  const rels=relationData(),from=currentPlaceId();if(!rels.length||!from)return;
  const candidates=[...document.querySelectorAll('#placesContent .places-section')].filter(sec=>{const h=sec.querySelector('h2');return h&&/à proximité|relié|autour du hall/i.test(h.textContent||'')});
  for(const sec of candidates){for(const row of sec.querySelectorAll('.place-row[data-place]')){const to=row.dataset.place||'';if(!to)continue;const hasForward=rels.some(r=>r.from_place_id===from&&r.to_place_id===to);if(hasForward)continue;const rev=rels.find(r=>r.to_place_id===from&&r.from_place_id===to&&(r.direction||r.label));if(!rev)continue;const ps=[...row.querySelectorAll('.place-row-main p')],last=ps.at(-1);if(!last)continue;const lt=norm(last.textContent),rd=norm(rev.direction||''),rl=norm(rev.label||'');if((rd&&lt.includes(rd))||(rl&&lt.includes(rl)))last.remove()}}
}
function enhanceDirections(){
  const content=q('#placesContent');if(!content||content.querySelector('[data-visit-directions]')||!relationData().length)return;
  const candidates=[...content.querySelectorAll('.places-section')].filter(sec=>{const h=sec.querySelector('h2');return h&&/à proximité|relié|autour du hall|depuis cette entrée/i.test(h.textContent||'')});
  for(const sec of candidates){
    const buckets={left:[],straight:[],right:[]},seen=new Set();for(const row of [...sec.querySelectorAll('.place-row[data-place]')]){const key=row.dataset.place||'';if(seen.has(key))continue;const d=directionForRow(row);if(d){seen.add(key);buckets[d].push(row)}}
    const used=Object.values(buckets).flat();if(!used.length)continue;
    const wrap=document.createElement('section');wrap.className='places-section visit-direction-section';wrap.dataset.visitDirections='1';wrap.innerHTML='<header class="places-section-head"><h2>Depuis ici</h2><small>Seulement les directions observées depuis ce point</small></header><div class="visit-direction-grid"></div>';
...
function enhanceArrivalLevel(){
  const id=currentPlaceId(),a=arrival(),content=q('#placesContent'),hero=content?.querySelector('.place-hero-card');if(!hero)return;const kicker=norm(hero.querySelector('.place-kicker')?.textContent);if(kicker!=='niveau')return;
  content.querySelector('[data-visit-arrival]')?.remove();
  if(!a||a.levelId!==id)return;
  const b=document.createElement('div');b.className='visit-arrival';b.dataset.visitArrival='1';b.innerHTML=`<strong>Sortie d’ascenseur professionnel</strong><span>Tu arrives par ${a.elevatorName?String(a.elevatorName).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])):'l’ascenseur professionnel sélectionné'}.</span>`;hero.after(b);
...
function enhanceDirectLevel(){
  const content=q('#placesContent'),hero=content?.querySelector('.place-hero-card');if(!hero||content.querySelector('[data-visit-arrival-choice]'))return;const kicker=norm(hero.querySelector('.place-kicker')?.textContent);if(kicker!=='niveau')return;const a=arrival();if(a&&a.levelId===currentPlaceId())return;
  const sec=[...content.querySelectorAll('.places-section')].find(s=>{const h=s.querySelector('h2');return h&&/à proximité|relié/i.test(h.textContent||'')});if(!sec)return;const elevators=[...sec.querySelectorAll('.place-row[data-place]')].filter(isProfessionalElevatorRow);if(!elevators.length)return;
  const pick=title('D’où arrives-tu ?','Choisis uniquement un ascenseur professionnel connu pour orienter la sortie','visit-arrival-choice');pick.dataset.visitArrivalChoice='1';elevators.forEach(r=>pick.querySelector('.places-list').appendChild(r));sec.before(pick);if(!sec.querySelector('.place-row[data-place]'))sec.remove()
}
function enhanceIncomplete(){
  const content=q('#placesContent'),hero=content?.querySelector('.place-hero-card');if(!hero)return;const kicker=norm(hero.querySelector('.place-kicker')?.textContent),id=currentPlaceId();
  ...
  if((kicker==='entrée'||kicker==='entree')&&!content.querySelector('.place-row[data-place]')&&!content.querySelector('[data-visit-entry-missing]')){const m=document.createElement('div');m.className='visit-missing';m.dataset.visitEntryMissing='1';m.innerHTML=`<strong>À compléter</strong><span>${id==='hfme_entry_main'?'Le parcours immédiatement après l’entrée principale HFME reste partiel. Les ascenseurs visiteurs servent uniquement de repères ; l’accès professionnel doit être relevé séparément.':'Le parcours immédiatement après cette entrée reste à compléter.'}</span>`;hero.after(m)}
}
function run(){markVisit();enhanceHospital();enhanceContextHeadings();sanitizeReverseDirections();enhanceDirections();enhanceArrivalLevel();enhanceDirectLevel();enhanceIncomplete()}
window.addEventListener('hashchange',()=>setTimeout(run,80));window.addEventListener('stip:place-relations-ready',()=>setTimeout(run,20));window.addEventListener('stip:place-data-ready',()=>setTimeout(run,20));
