...
function enhanceHospital(){if(!visitMode())return;const content=q('#placesContent');if(!content||content.querySelector('.visit-nav-section'))return;const hero=content.querySelector('.place-hero-card');if(!hero)return;const kicker=norm(hero.querySelector('.place-kicker')?.textContent);if(!kicker.includes('hôpital')&&!kicker.includes('hopital'))return;
 ...
 old.remove();
 const p=hero.querySelector('p');if(p&&!hero.querySelector('.visit-nav-help')){const h=document.createElement('p');h.className='place-details-note visit-nav-help';h.textContent='Commence par un ascenseur pour parcourir rapidement le bâtiment. Les étages restent disponibles juste en dessous, du plus bas au plus haut.';hero.appendChild(h)}
}
