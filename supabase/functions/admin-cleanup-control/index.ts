...
async function decide(actor: string, body: any) {
  ...
  if (local) {
    ...
    await db.from("admin_cleanup_case_events").insert({
      case_id: id, event_type: "execution_completed", actor: "admin-cleanup-control", payload: result
    });
    return done;
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "request_scan") return J({ ok: true, run: await requestScan(admin.email, body) });
    if (action === "settings") return J({ ok: true, settings: await updateSettings(body.patch || {}) });
    if (action === "decision") return J({ ok: true, case: await decide(admin.email, body) });
