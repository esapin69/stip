...
function resolveShift(defs:any[],raw:any){const src=String(raw||'').trim().toUpperCase(),clean=src.replace(/\*+$/,'');if(!clean)return null;let row=defs.find((x:any)=>String(x.code||'').toUpperCase()===clean);if(row)return row;let base='';if(/^J4\d+$/.test(clean))base='J4';else if(/^M\d+$/.test(clean))base='M';else if(/^J\d+$/.test(clean))base='J';else if(/^S\d+$/.test(clean))base='S';else if(/^N\d+$/.test(clean))base='N';return base?defs.find((x:any)=>String(x.code||'').toUpperCase()===base)||null:null}
const PRIVATE_DIRECTORY_CODES=new Set(['AR','AT','MA','AM','AA','ABS','SYR']);
function safeDirectoryItem(a:any,canFull:boolean,canContacts:boolean){
  const code=String(a?.today_code||'').toUpperCase();
  const safeCode=!canFull&&PRIVATE_DIRECTORY_CODES.has(code)?'ABS':a?.today_code;
  return {
    ...a,
    today_code:safeCode,
    today_observation:canFull?(a?.today_observation||null):null,
    ...(!canContacts?{telephone:null,email:null}:{}),
  };
}
async function directory(){const today=parisDay();const [aq,pq,shiftDefs]=await Promise.all([db.from('agents').select('id,source_key,nom,prenom,equipe,type_planning,ghe,telephone,email,avatar_url,role,actif').eq('actif',true).order('nom'),db.from('planning').select('date,code,observation,equipe,agent_id,agent_source_key').eq('date',today),shiftDefinitions()]);if(aq.error)throw aq.error;if(pq.error)throw pq.error;const byId=new Map<string,any>(),byKey=new Map<string,any>();for(const p of pq.data||[]){if(p.agent_id)byId.set(String(p.agent_id),p);if(p.agent_source_key)byKey.set(String(p.agent_source_key),p)}const items=(aq.data||[]).map((a:any)=>{const row=byId.get(String(a.id))||byKey.get(String(a.source_key)),code=String(row?.code||'').toUpperCase()||null,definition=resolveShift(shiftDefs,code),special=definition&&definition.schedule_mode!=='standard'?definition:null;return{...a,today_code:code,today_observation:row?.observation||null,today_team:row?.equipe||null,today_shift_definition:definition,today_special_schedule:special,is_working:Boolean(definition?.is_working)}});return{date:today,items,shift_definitions:shiftDefs,counts:{total:items.length,working:items.filter((x:any)=>x.is_working).length,not_working:items.filter((x:any)=>!x.is_working).length}}}
Deno.serve(async req=>{if(req.method==='OPTIONS')return new Response('ok',{headers:C});if(req.method!=='POST')return J({error:'Méthode non autorisée.'},405);try{const b=await req.json().catch(()=>({})),action=String(b.action||'');if(action==='directory'){const v:any=await viewer(req),d:any=await directory(),canContacts=Boolean(v.permissions?.contacts||v.permissions?.equipe_contacts||v.permissions?.responsable||v.permissions?.admin),canFull=Boolean(v.permissions?.responsable||v.permissions?.admin||/chef|responsable|cadre/i.test(String(v.agents?.role||'')));return J({...d,items:(d.items||[]).map((a:any)=>safeDirectoryItem(a,canFull,canContacts)),viewer_permissions:v.permissions||{},viewer:{can_contacts:canContacts,can_full:canFull}})}if(action==='issue_grant'){const v:any=await viewer(req),canFull=Boolean(v.permissions?.responsable||v.permissions?.admin||/chef|responsable|cadre/i.test(String(v.agents?.role||'')));if(!canFull)throw Error('Fiche STIP complète réservée aux profils autorisés.');const id=String(b.agent_id||'');if(!id)throw Error('Agent manquant.');await target(id);const raw=randomToken(),hash=await sha256(raw),expires=new Date(Date.now()+30*60*1000).toISOString();await db.from('stip_readonly_grants').delete().eq('viewer_profile_id',v.id).lt('expires_at',new Date().toISOString());const {error}=await db.from('stip_readonly_grants').insert({token_hash:hash,viewer_profile_id:v.id,target_agent_id:id,expires_at:expires});if(error)throw error;return J({grant:raw,expires_at:expires})}const g:any=await grant(req);if(action==='grant_boot')return J(await boot(g.target_agent_id,g.viewer_permissions));if(action==='grant_team')return J(await team(g.target_agent_id,g.viewer_permissions));if(action==='grant_contacts')return J(await contacts(g.viewer_permissions));return J({error:'Action invalide.'},400)}catch(e){console.error(e);return J({error:e instanceof Error?e.message:String(e)},400)}})
