...
function publish(rows){if(!window.STIPExchange||!window.STIPRetain||!window.STIPTimeline)return;
 ...
 for(const x of rows){const id=String(x.id||'');if(!id)continue;const label=typeLabel(x),date=String(x.date_from||'').slice(0,10),status=statusLabel(x);
  ...
  const a=x.context?.analysis||x.analysis;if(a?.level==='risk'&&!closed(x)){const k='dw-risk:'+id,critical=Number(a.severity||0)>=4||String(a.level||'').toLowerCase()==='critical',icon=critical?'🛑':'⚠️',title=critical?'Ça coince si ce changement passe':'Ce changement fragilise la journée',lead=a.message||'L’effectif devient plus fragile sur au moins un créneau.';rt.add(k);window.STIPRetain.publish({id:k,icon,title,lead,meta:date?fmt(date):'',href:'personal',priority:critical?96:90,source:'day-workflow'});}
