(() => {
  ...
  let allItems = [],
    activeCategory = navigationState.category || "Tous",
  ...
  function filtered() {
    return allItems.filter(
      (item) =>
        (activeCategory === "Tous" ||
          (item.category || "Documents") === activeCategory) &&
  ...
  function renderFilters() {
    const cats = [
      "Tous",
      ...[...new Set(allItems.map((x) => x.category || "Documents"))].sort(
        (a, b) => a.localeCompare(b, "fr"),
      ),
    ];
    $("#documentsFilters").innerHTML = cats
      .map(
        (c) =>
          `<button type="button" class="doc-filter ${c === activeCategory ? "active" : ""}" data-category="${esc(c)}">${esc(c)}</button>`,
      )
      .join("");
    document.querySelectorAll("[data-category]").forEach(
      (b) =>
        (b.onclick = () => {
          ...
          window.STIPNav?.remember?.({ category: activeCategory });
          renderFilters();
          renderList();
        }),
    );
  ...
  function renderList() {
    ...
    $("#documentsCount").textContent = allItems.length;
    $("#documentsHeadline").textContent = allItems.length
      ? `${allItems.length} document${allItems.length > 1 ? "s" : ""} disponible${allItems.length > 1 ? "s" : ""}`
      : "Vos documents";
    $("#documentsSummary").textContent = allItems.length
      ? "Recherchez directement une ressource ou filtrez par catégorie."
      : "Aucun document n’est actuellement mis à disposition.";
    if (!items.length) {
      box.innerHTML = `<div class="documents-empty">${allItems.length ? "Aucun document ne correspond à cette recherche." : "Aucun document n’est actuellement mis à disposition."}</div>`;
      return;
    }
    const groups = new Map();
    for (const item of items) {
      const k = item.category || "Documents";
  ...
  async function boot() {
    ...
    try {
      ...
      const j = await post(API, { action: "documents" });
      ...
      $("#error").textContent =
        e.message || "Impossible de charger les documents.";
    }
  }
  $("#documentsSearch").addEventListener("input", (e) => {
    query = e.target.value.trim().toLowerCase();
    window.STIPNav?.remember?.({ search: query });
    renderList();
  });
  $("#documentsSearch").value = query;
  window.STIPNav?.register?.({
    capture: () => ({ category: activeCategory, search: query }),
  });
  boot();
})();
