...
The canonical list of **possible dictionary fields** is Supabase `stip_place_dictionary_field_defs`.
It defines what a complete destination could contain, independently from whether a given service currently has the value.

Non-negotiable invariants:
- `places.html`, Chat STIP / Fauteuils and human/file exports consume the same Supabase referential. Do not create or restore a parallel catalogue in page JavaScript, JSON, Sheet, Drive or PDF.
...
- The professional interface may display missing expected fields as **« À compléter »**. These placeholders are collection aids only and are not exportable content.
- PDF/share output automatically omits empty fields and placeholder-only values such as `À compléter`, `À confirmer`, `À préciser` and `Information non renseignée`.
- Chat Fauteuils obtains its location catalogue from `stip-messages · wheelchair_catalog`; wheelchair-only metadata may remain local only when genuinely specific to that workflow.
- In Chat Fauteuils, shortcut buttons may stay curated for speed, but **Service / repère must search the entire canonical `stip_places` referential**, including annexes and non-shortcut buildings. Do not filter the search catalogue down to the shortcut buildings.
- Current curated quick places are Neuro, Cardio, HFME, POP/A4 and **Médecine nucléaire / B14**; other buildings remain reachable through global Service / repère search without adding visual clutter.
- XLSX and PDF exports are generated by `stip-places` from the same canonical referential used by Visiter les lieux.
...
- Supabase Edge Function source remains versioned in GitHub under `supabase/functions/*`. Before declaring a backend change complete, verify deployed source matches GitHub.
- Add or extend regression checks whenever this contract changes.

## 15. Visiter les lieux — PDF généré, templates STIP et dictionnaires

Le PDF opérationnel suit une architecture hybride : **pages fixes de repérage + dictionnaires dynamiques**.

Règles validées :
- Le site **fabrique chaque PDF à la demande**. Aucun fichier Drive n’est nécessaire au fonctionnement de l’outil.
- Les anciennes ressources Drive peuvent rester archivées comme historique humain, mais elles ne sont plus une dépendance de génération, de téléchargement, de partage ou de mise à jour.
- La base visuelle fixe utilisée par le moteur est stockée **dans le stockage privé Supabase STIP** (`ghe-media`). Le code la désigne comme `FIXED_TEMPLATE_PATH`.
- Les données métier et les champs attendus du dictionnaire sont dans Supabase ; le PDF n’est jamais la source de vérité.
- Le catalogue `stip_place_dictionary_field_defs` contient actuellement les familles attendues : identification, localisation, accès/transport, contacts/horaires, informations opérationnelles, source/confirmation.
- Une fiche peut donc être très incomplète dans STIP tout en montrant clairement ce qui reste à renseigner. Lors de l’export, **seuls les champs réellement renseignés sont composés**.
- Les actions d’export sont **Afficher / Télécharger / Partager**. Le bouton autonome **Imprimer** ne doit pas revenir ; l’impression éventuelle se fait depuis le lecteur du PDF affiché.
...
- Le nombre final de pages n’est pas figé : les dictionnaires paginent selon le périmètre et la richesse réelle des données.
- Les trois bâtiments principaux suivent l’ordre cible : **intercalaire fixe/généré → page visuelle fixe → dictionnaire dynamique**.
- Les trois bâtiments principaux sont : **NEURO (Pierre Wertheimer) · CARDIO (Louis Pradel) · HFME (Femme Mère Enfant)**.
- Les bâtiments annexes utilisent une section dominante par bâtiment, suivie de leur dictionnaire plus discret, sans ambiguïté d’appartenance.
...
- **Aucune information opérationnelle utile n’est supprimée pour faire tenir le PDF.** On optimise sa présentation.
...
Le catalogue des champs attendus doit pouvoir absorber, lorsqu’ils existent et sont utiles :
- nom officiel, alias, nom de mission, codes, PTAH, UF ;
- bâtiment, étage, aile/secteur, unité, salle/chambre, plages de chambres ;
- adresse, transport/arrêt ;
- itinéraire, ascenseur, badge/accès professionnel, lit/fauteuil/brancard, accessibilité, repère d’arrivée, proximité, organisation interne, raccourci/liaison ;
- téléphone public, contacts internes, secrétariat, e-mail, horaires d’accueil, horaires de rendez-vous ;
- équipe/fonctions utiles, contacts associés, collaborations ;
- points de vigilance, particularités, prise en charge particulière, informations temporaires, notes terrain, repères/équipements ;
- source, date et niveau de confirmation.

Les mentions `à compléter`, `à confirmer`, `à préciser`, `provisoire`, etc. sont des **lacunes**, jamais des faits à republier comme certains.

Le générateur PDF sépare :
1. la base visuelle fixe stockée dans STIP ;
2. la sélection canonique des données ;
3. le catalogue des champs attendus ;
4. le template compact de dictionnaire ;
