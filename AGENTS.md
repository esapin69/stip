...
- In `J’ai vu` mode, an availability signal must be recoverable in the field: building/floor alone or a bare generic clue such as `Ascenseur`, `Couloir`, `Escalier`, `Hall` or `Accueil` is not sufficient. Require a canonical service/unit/landmark from `stip_places` or an explicit precision before sending. Legacy vague signals must be labelled as imprecise; never invent a missing unit or service.
