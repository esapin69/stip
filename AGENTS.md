...
- XLSX and PDF exports are generated by `stip-places` from the same bootstrap data used by Visiter les lieux. PDF generation uses the validated fixed-page template bank from the private MASTER mirror plus dynamic dictionaries built from Supabase. `places-export.html` is only the UI trigger and must not contain a second data model.
...
## 15. Visiter les lieux — PDF généré, pages fixes et dictionnaires
