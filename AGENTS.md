...
- XLSX exports are generated by `stip-places` from the same bootstrap data used by Visiter les lieux. The current complete PDF is the official Drive MASTER and must not be regenerated from Supabase. Filtered/dynamic PDF export stays disabled until the dictionary template is explicitly validated. `places-export.html` is only the UI trigger and must not contain a second data model.
