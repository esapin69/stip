(()=>{'use strict';
const baseFetch=window.fetch.bind(window),active=new Map(),inflight=new Map();let hud=null,hideTimer=null,seq=0;
...
if(isData&&action==='contacts'&&route()==='home'&&!window.STIPContactsUserRequested){return Promise.reject(new DOMException('Chargement différé jusqu’à l’ouverture des contacts.','AbortError'))}
...
const out=await run;if(out.data!==null)return out.response;return out.response};
window.STIPReadCache={clear(action=''){try{const prefix=CACHE_PREFIX+sessionKey()+':'+action;for(let i=sessionStorage.length-1;i>=0;i--){const k=sessionStorage.key(i)||'';if(k.startsWith(prefix))sessionStorage.removeItem(k)}}catch{}},requestContacts(){window.STIPContactsUserRequested=true}};
