(() => {
  ...
  const APP_POLICY = {
    ...
    access: () => explicit("access_manage") || explicit("admin"),
    ...
    livePermRefresh = (async () => {
      try {
        ...
        window.STIPSession = {
          ...
          depths: {
            ...((window.STIPSession || {}).depths || {}),
            ...(j.depths || {}),
          },
        };
        if (window.STIPBootCache)
          window.STIPBootCache = {
            ...window.STIPBootCache,
            permissions: {
              ...(window.STIPBootCache.permissions || {}),
              ...j.permissions,
        ...
        window.dispatchEvent(
          new CustomEvent("stip:permissions-live", { detail: j.permissions }),
        );
        return j.permissions;
      } catch {
        return null;
      } finally {
        livePermRefresh = null;
      }
    })();
    return livePermRefresh;
  }
  function normalizeInfoLevel(v) {
    ...
    if (
      ...
      return "pro";
    if (v === "visitor" || v === "visiteur" || v === "public" || v === "basic")
      return "visitor";
    return "none";
  }
  function infoLevel(app = "") {
    const lv = p().__levels?.[app];
    if (lv) return normalizeInfoLevel(lv);
    const n = normalizeInfoLevel(depth(app));
    return n !== "none" ? n : "visitor";
  }
  function canInfo(level = "visitor", app = "") {
    if (app && !explicit(app)) return false;
    const wanted = normalizeInfoLevel(level),
      current = infoLevel(app);
    return wanted === "visitor"
      ? current === "visitor" || current === "pro"
      : wanted === "pro" && current === "pro";
  }
  function applyInfoPolicy(root = document) {
    for (const el of root.querySelectorAll(
      "[data-stip-info],[data-access-level]",
    )) {
      const wanted = el.dataset.stipInfo || el.dataset.accessLevel || "visitor",
        app =
          el.dataset.stipApp ||
          el.closest("[data-stip-app]")?.dataset.stipApp ||
          "";
      el.hidden = !canInfo(wanted, app);
    }
  }
  function guardRoute() {
    const r = window.STIPRouter?.get?.() || "";
    let ok = true;
    if (r === "planning/personal") ok = canApp("personal");
    else if (r === "planning/calendar") ok = canApp("calendar");
  ...
  function guardStandalone() {
    const path = location.pathname.toLowerCase();
    if (
      path.endsWith("/esprit-equipe.html") &&
      !["team", "activity", "assistant"].some(canApp)
    ) {
      location.replace("./");
      return false;
    }
    const checks = [
      ...
      ["/planning-compare-app.html", "compare"],
      ["/places.html", "places"],
      ["/places-app.html", "places"],
      ["/access-manage.html", "access"],
    ];
    for (const [needle, key] of checks) {
      if (path.endsWith(needle) && !canApp(key)) {
        location.replace("./");
        return false;
      }
    }
  ...
  const ICON = {
    ...
    notes:
      '<svg viewBox="0 0 24 24"><path d="M4 20h4l11-11-4-4L4 16v4Z"/><path d="m13.5 6.5 4 4M4 20l4-1"/></svg>',
    newagent:
    ...
    activity:
      '<svg viewBox="0 0 24 24"><path d="M4 16l4-5 4 3 5-7 3 2"/><path d="M4 20h16"/></svg>',
    admin:
      '<svg viewBox="0 0 24 24"><path d="M12 3 4 7v10l8 4 8-4V7z"/><path d="M9 12h6M12 9v6"/></svg>',
    places:
      '<img src="images/icone_app/visiter-les-lieux.webp?v=20260831-icons3" alt="" aria-hidden="true">',
    assistant: "🦜",
    access: "🔐",
  };
  const SPECS = [
    ...
    ["dates", "Date des agents", "dates"],
    ["contacts", "Contacts", "contacts"],
    ["responsable", "Responsable", "responsable"],
    ["notes", "Prendre des notes", "notes"],
    ["newagent", "Nouvel agent", "newagent"],
  ...
  function dedupe(box) {
    const seen = new Set();
    for (const b of [...box.querySelectorAll(".hc-app[data-app]")]) {
  ...
  function ensureCard(box, key, title, cls) {
    let b = box.querySelector(`[data-app="${key}"]`);
    if (!canApp(key)) {
      box.querySelectorAll(`[data-app="${key}"]`).forEach((x) => x.remove());
      return null;
    }
    if (!b) {
      b = document.createElement("button");
      b.type = "button";
      b.className = `hc-app ${cls}`;
      b.dataset.app = key;
      b.innerHTML = `<span>${ICON[key] || ""}</span><strong>${title}</strong>`;
    }
    b.style.display = "";
    b.onclick = (e) => openApp(key, e);
    return b;
  }
  function applyHomePolicy() {
    const box = document.querySelector("#homeView .hc-apps");
    if (!box) return;
    box.querySelectorAll('[data-app="equipe"]').forEach((b) => b.remove());
    dedupe(box);
    for (const b of [...box.querySelectorAll("[data-app]")]) {
      const k = b.dataset.app;
      if (k in APP_POLICY && !canApp(k)) b.remove();
    }
    const wanted = [];
    for (const [key, title, cls] of SPECS) {
      const b = ensureCard(box, key, title, cls);
      if (b) wanted.push(b);
    }
    dedupe(box);
    const current = [...box.querySelectorAll(".hc-app")];
    const stable =
      wanted.length === current.length &&
      wanted.every((b, i) => current[i] === b);
    if (!stable) {
      const frag = document.createDocumentFragment();
      wanted.forEach((b) => frag.appendChild(b));
      box.replaceChildren(frag);
    }
    const empty = box.querySelector(".hc-empty");
    if (empty && wanted.length) empty.remove();
    if (!wanted.length && !box.querySelector(".hc-empty"))
      box.innerHTML = '<p class="hc-empty">Aucune application autorisée.</p>';
  }
  function tuneDock() {
    for (const b of document.querySelectorAll(
      "#stipContextDock [data-root-action]",
    )) {
      const a = b.dataset.rootAction;
      let ok = true;
      if (a === "calendar" || a === "personalcal") ok = canApp("calendar");
      if (a === "teamcal")
        ok = canApp("team") && explicit("calendar_subscribe");
      if (a === "contact-share") ok = canApp("contacts");
      b.style.display = ok ? "" : "none";
    }
  }
  function run() {
    if (!guardStandalone()) return;
    guardRoute();
    applyHomePolicy();
    tuneDock();
    applyInfoPolicy();
  ...
  window.addEventListener("stip:session-ready", () =>
    refreshLivePermissions().then(schedule),
  );
