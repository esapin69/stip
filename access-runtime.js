...
function explicit(k){const x=p();return Object.prototype.hasOwnProperty.call(x,k)?!!x[k]:null}
function allow(primary,...fallbacks){const v=explicit(primary);if(v!==null)return v;for(const k of fallbacks){const f=explicit(k);if(f!==null)return f}return false}
const APP_POLICY={
...
function run(){guardRoute();applyHomePolicy();tuneDock();document.documentElement.dataset.stipRole=window.STIPSession?.role_key||'';document.documentElement.dataset.equipeDepth=depth('equipe');document.documentElement.dataset.contactsDepth=depth('contacts')}
new MutationObserver(()=>requestAnimationFrame(run)).observe(document.body,{childList:true,subtree:true});['stip:session-ready','stip:boot-updated','stip:route','stip:lazy-ready'].forEach(e=>window.addEventListener(e,()=>requestAnimationFrame(run)));window.STIPAccess={has:k=>allow(k),allow,app:canApp,depth,can:k=>allow(k)};run()})();