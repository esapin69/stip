...
function shortElevatorName(name=''){return String(name).replace(/^Ascenseurs?\s+(professionnels?\s*)?[—–-]?\s*/i,'').trim()||'Ascenseur pro'}
function render(){
  ...
  let buttons='',mode='levels',title='Étages',subtitle=p.building_code||'',signature='',contextName='';
  if(elevator){const stops=effectiveStops(elevator,byParent,tags);if(stops.length){mode='elevator';title='Ascenseur pro';contextName=shortElevatorName(elevator.display_name);subtitle=contextName;buttons=elevatorButtons(stops,active);signature=`e:${elevator.id}:${active?.id||''}:${stops.map(s=>`${s.stop_label}:${s.linked_place_id||''}`).join('|')}`}}
  ...
  rail.innerHTML=`<button type="button" class="floor-quick-handle" data-floor-toggle aria-expanded="${opened?'true':'false'}" aria-label="${opened?'Fermer':'Ouvrir'} ${esc(title.toLowerCase())}"><span class="floor-handle-live"></span><small>${mode==='elevator'?'PRO':'NIVEAUX'}</small><strong>${esc(compactFloor(active))}</strong><span class="floor-handle-chevron">${opened?'›':'‹'}</span></button><section class="floor-quick-panel" aria-hidden="${opened?'false':'true'}"><header><div><small>${mode==='elevator'?'ACCÈS VERTICAL':'ACCÈS RAPIDE'}</small><strong>${esc(title)}</strong>${subtitle?`<span>${esc(subtitle)}</span>`:''}</div><button type="button" class="floor-panel-close" data-floor-toggle aria-label="Fermer">×</button></header><div class="floor-panel-grid">${buttons}</div>${mode==='elevator'?'<p>Uniquement les arrêts documentés de cet ascenseur professionnel.</p>':'<p>Choisis directement un étage.</p>'}</section>`;
